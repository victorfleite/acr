...
abstract class base_optimization {
    ...
    public function get_posttracking_report() {
        ...
        $rows = $report->getReport(
                'urn:Adition/Tracking/PostReport2', $this->row['postreport_startdate'], $today, $params, $this->row['network_id'], $params, $period, 'bw'
        );
    ...
    protected function writeLog($data) {
      ...
      VALUES (%s, %s, %s, %s, %s, %s)', $this->db->quote($data['campaign_data']['order_id'], 'integer'), $this->db->quote($data['network_id'], 'integer'), $this->db->quote($data['campaign_data']['id'], 'integer'), $this->db->quote($logMsg, 'text'), $this->db->quote(serialize($data), 'text'), $this->db->quote($lastrun, 'timestamp')
        );
